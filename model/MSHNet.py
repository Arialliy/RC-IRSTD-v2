...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        # Reductions are mathematically identical to adaptive pooling to 1x1,
        # while avoiding CUDA's non-deterministic adaptive-max backward kernel.
        avg_out = self.fc2(self.relu1(self.fc1(x.mean(dim=(-2, -1), keepdim=True))))
        max_out = self.fc2(self.relu1(self.fc1(x.amax(dim=(-2, -1), keepdim=True))))
        out = avg_out + max_out
