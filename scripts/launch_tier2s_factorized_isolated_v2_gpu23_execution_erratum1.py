#!/usr/bin/env python3
"""Fail-closed Tier2S launcher with append-only execution erratum1.
...
The frozen v2 protocol, governance amendment, code, tests, and failed Tier2R
evidence remain unchanged.  This launcher corrects only two post-registration
execution-contract defects: an unreachable strict-zero-MiB idle predicate on
this host and the obsolete pre-registration-only test state.  It can never
authorize Tier3, paper claims, RiskCurve, or NUAA access.
"""
...
from scripts import register_tier2s_gpu23_execution_erratum1 as execution_erratum  # noqa: E402


PROTOCOL_ID = "tier2s_factorized_causal_audit_v2_gpu23"
RESEARCH_MODE = "exploratory_source_only"
COORDINATOR_SCHEMA = "rc-irstd-aaai27-tier2s-factorized-coordinator-v2-gpu23"
PROBE_SCHEMA = "rc-irstd-aaai27-tier2s-factorized-container-attestation-v2-gpu23"
COORDINATOR = PROJECT_ROOT / "scripts/coordinate_tier2s_factorized_audit_v2_gpu23.py"
CONTAINER_PROBE = PROJECT_ROOT / "scripts/probe_tier2s_factorized_container_v2_gpu23.py"
EXPORTER = PROJECT_ROOT / "scripts/export_tier2s_factorized_logits_v2_gpu23.py"
EVALUATOR = PROJECT_ROOT / "scripts/evaluate_tier2s_factorized_audit_v2_gpu23.py"
GOVERNANCE_REGISTRAR = PROJECT_ROOT / "scripts/register_tier2s_gpu23_amendment_v2.py"
EXECUTION_ERRATUM_REGISTRAR = (
    PROJECT_ROOT / "scripts/register_tier2s_gpu23_execution_erratum1.py"
)
PROTOCOL = PROJECT_ROOT / "configs/tier2s_factorized_causal_audit_v2_gpu23.json"
EXECUTION_ERRATUM_CONFIG = PROJECT_ROOT / "configs/tier2s_gpu23_execution_erratum1.json"
PARENT_LAUNCHER = (
    PROJECT_ROOT / "scripts/launch_tier2s_factorized_isolated_v2_gpu23.py"
)
...
INTENT_PATH = AUDIT_ROOT / "LAUNCH_INTENT_EXECUTION_ERRATUM1.json"
INTENT_SHA256_PATH = AUDIT_ROOT / "LAUNCH_INTENT_EXECUTION_ERRATUM1.json.sha256"
...
PARENT_GOVERNANCE_REGISTRATION_RELATIVE = Path(
...
DOCKER_GPU_REQUEST = '"device=2,3"'
NVIDIA_VISIBLE_DEVICES = "2,3"
CONTAINER_NAME = (
    "rc-irstd-tier2s-factorized-causal-audit-v2-gpu23-execution-erratum1"
)
SCHEMA = (
    "rc-irstd-aaai27-tier2s-factorized-isolated-launch-intent-v2-gpu23-execution-erratum1"
)
INTENT_LABEL = base.INTENT_LABEL
DRIVER_BOOKKEEPING_MAX_MIB = 4
IDLE_SCHEMA = "rc-irstd-aaai27-tier2s-gpu-idle-preflight-v2-gpu23-execution-erratum1"
...
RUNTIME_LABELS = {
    ...
    "org.rc-irstd.execution-erratum": "driver-bookkeeping-max-4mib",
}
...
CONFIG_PATHS = (PROTOCOL, EXECUTION_ERRATUM_CONFIG)
...
def _require_governance_binding(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    expected_keys = {
        "schema_version",
        "registration",
        "parent_governance_registration",
        "contract",
        "fresh_seed_ledger",
        "fresh_seed_local_scan",
        "code_sha256_canonical_sha256",
        "physical_gpus",
        "container_logical_ordinals",
    ...
    if (
        ...
        or binding.get("physical_gpus") != [2, 3]
        or binding.get("container_logical_ordinals") != [0, 1]
        or binding.get("physical_to_container_ordinal") != {"2": 0, "3": 1}
        or binding.get("gpu_fallback_allowed") is not False
        ...
        or binding.get("paper_claim_authorized") is not False
    ...
    _require_frozen_artifact_binding(
        binding.get("parent_governance_registration"),
        expected_path=PARENT_GOVERNANCE_REGISTRATION_RELATIVE,
        name="parent governance registration",
    )
    _require_frozen_artifact_binding(
    ...
    superseded = binding.get("superseded_execution_plan")
    if (
        not isinstance(superseded, Mapping)
        or superseded.get("protocol_path")
        != "configs/tier2s_factorized_causal_audit_v1.json"
        or superseded.get("protocol_sha256")
        != "9f0ec09e0289409b184e6dec1fe89b2a51ad5fc483f4f0ca5d27195173cdf129"
        or superseded.get("output_namespace_absent") is not True
        or superseded.get("audit_namespace_absent") is not True
        or superseded.get("historical_evidence_superseded") is not False
        or superseded.get("unexecuted_hardware_plan_replaced_only") is not True
    ):
        raise RuntimeError("Tier2S superseded GPU0/1 execution-plan binding drift")
    return json.loads(
...
def _coordinator_contract(payload: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    ...
    expected_lanes = {"2": 9, "3": 9}
    expected_mapping = {"2": 0, "3": 1}
    if (
        ...
        or payload.get("physical_to_container_ordinal") != expected_mapping
    ):
    ...
    if (
        ...
        or any(
            ...
            or value.get("container_gpu_ordinal")
            != CONTAINER_ORDINAL_BY_PHYSICAL.get(value.get("physical_gpu"))
...
def _assert_exact_gpu_command(command: Sequence[str]) -> None:
    ...
    if (
        ...
        raise RuntimeError(
            "Tier2S Docker GPU request must be exactly device=2,3, never all"
        )
    environment_values = [
        command[index + 1]
...
def _selected_host_gpus(
    ...
    if any(index not in by_index for index in TARGET_GPU_INDICES):
        raise RuntimeError("required physical GPU 2/3 is absent")
...
def _verify_container_attestation(
    ...
    *,
    ...
    if payload.get("nvidia_inventory") != expected_nvidia:
        problems.append("container NVIDIA inventory must be exact physical 2/3")
    if payload.get("torch_inventory") != expected_torch:
        problems.append("Torch ordinals must map exactly to host physical 2/3")
...
def _verify_selected_gpus_idle(
    ...
    *,
    ...
    if observed_identity != expected_identity:
        raise RuntimeError(
            "selected-GPU idle query did not return exact physical GPU 2/3 order"
        )
    non_idle = [
        value
        for value in stats
        if value["memory_used_mib"] > DRIVER_BOOKKEEPING_MAX_MIB
    ]
    process_command = [
        "nvidia-smi",
        "-i",
    ...
    if non_idle or processes:
        raise RuntimeError(
            "selected physical GPU 2/3 exceeds driver-only idle contract"
        )
    return {
        "schema_version": IDLE_SCHEMA,
        "verified": True,
...
def verify_only(*, runner: base.Runner | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "execution_erratum_governance_binding": erratum_binding,
...
def _launch_intent_payload(
    *,
    ...
    execution_erratum_binding = _require_execution_erratum_binding()
    ...
    if (
        idle_preflight.get("schema_version") != IDLE_SCHEMA
        or idle_preflight.get("verified") is not True
        ...
        or idle_preflight.get("driver_bookkeeping_ceiling_mib")
        != DRIVER_BOOKKEEPING_MAX_MIB
        or idle_preflight.get("no_compute_processes_required") is not True
    ):
    ...
    payload.update(
        {
            "schema_version": SCHEMA,
            "decision": "LAUNCH_TIER2S_FACTORIZED_SOURCE_ONLY_WITH_EXECUTION_ERRATUM1",
            "protocol_id": PROTOCOL_ID,
            "research_mode": RESEARCH_MODE,
            "source_tier3_authorized": False,
            "paper_claim_authorized": False,
            "governance_binding": verification_governance,
            "execution_erratum_governance_binding": execution_erratum_binding,
...
def _validate_existing_container(
    ...
    if not exact_request:
        drift.append("GPU exposure must be exact DeviceIDs 2,3")
    if drift:
        raise RuntimeError(
...
def execute_launch(*, runner: base.Runner | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "source_tier3_authorized": False,
        "paper_claim_authorized": False,
        "execution_erratum_governance_binding": evidence["execution_erratum_governance_binding"],
...
def dry_run_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "source_tier3_authorized": False,
        "paper_claim_authorized": False,
        "execution_erratum_config": str(EXECUTION_ERRATUM_CONFIG),
        "execution_erratum_registration": str(
            PROJECT_ROOT / EXECUTION_ERRATUM_REGISTRATION_RELATIVE
        ),
