#!/usr/bin/env python3
"""Append-only governance for Tier2S GPU2/3 execution erratum1.

The frozen v2 amendment, protocol, launcher, and tests remain immutable.  This
erratum freezes a separate launcher/test closure correcting only the host idle
predicate and the post-registration test phase transition.  It authorizes the
same source-only diagnostic on physical GPUs 2/3 and never authorizes V3,
Gate A, RiskCurve, paper claims, or outer-target access.
"""
...
from scripts import register_tier2s_gpu23_amendment_v2 as parent_v2  # noqa: E402


SCHEMA = "rc-irstd-aaai27-tier2s-gpu23-execution-erratum1-registration"
BINDING_SCHEMA = "rc-irstd-aaai27-tier2s-gpu23-execution-erratum1-binding"
...
PARENT_PROTOCOL = PROJECT_ROOT / "configs/tier2s_factorized_causal_audit_v2_gpu23.json"
PARENT_PROTOCOL_SHA256 = (
    "cf5630d1772965e5c326f30cf3b1023dd5e4458465238103c9801013bb80605c"
)
PARENT_OUTPUT_ROOT = (
    PROJECT_ROOT
...
ERRATUM_CONFIG = PROJECT_ROOT / "configs/tier2s_gpu23_execution_erratum1.json"
PARENT_LAUNCHER = PROJECT_ROOT / "scripts/launch_tier2s_factorized_isolated_v2_gpu23.py"
PARENT_LAUNCHER_SHA256 = (
    "f4da6156aaa7f8b5ffaea9d3e6e042e5a7faafd576eed6239b63eae49205cf6e"
)
OLD_V2_INTENT = (
    PROJECT_ROOT
    / "artifacts/aaai27/audit/source_rescue/tier2s_factorized_causal_audit_v2_gpu23/LAUNCH_INTENT.json"
)
AMENDMENT_ROOT = (
    PROJECT_ROOT
    / "artifacts/aaai27/audit/governance/tier2s_gpu23_execution_erratum1"
)
REGISTRATION = AMENDMENT_ROOT / "TIER2S_GPU23_EXECUTION_ERRATUM1.json"
...
def _verify_parent_plan_is_unexecuted() -> dict[str, Any]:
    ...
    if OLD_V2_INTENT.exists() or OLD_V2_INTENT.is_symlink():
        raise RuntimeError("superseded strict-zero-MiB v2 launch intent exists")
    ...
    return {
        ...
        "superseded_v2_launch_intent_absent": True,
        "parent_launcher_path": str(PARENT_LAUNCHER.relative_to(PROJECT_ROOT)),
        "parent_launcher_sha256": PARENT_LAUNCHER_SHA256,
        "historical_evidence_superseded": False,
        "replacement_scope": "idle_predicate_and_test_phase_transition_only",
    }


def verify_inputs() -> dict[str, Any]:
    parent = parent_v2.require_frozen_tier2s_governance(
        expected_registration_sha256=PARENT_REGISTRATION_SHA256
    )
    _require_regular(PROTOCOL, expected_sha256=PARENT_PROTOCOL_SHA256)
    _require_regular(ERRATUM_CONFIG)
    protocol = _load_json(PROTOCOL)
    erratum = _load_json(ERRATUM_CONFIG)
    ...
    idle = erratum.get("observed_post_registration_mismatches", {}).get(
        "idle_probe", {}
    )
    test_phase = erratum.get("observed_post_registration_mismatches", {}).get(
        "test_phase_transition", {}
    )
    unchanged = erratum.get("unchanged_scientific_and_hardware_contract", {})
    append_only = erratum.get("append_only_policy", {})
    if (
        erratum.get("schema_version")
        != "rc-irstd-aaai27-tier2s-gpu23-execution-erratum1"
        or erratum.get("erratum_id") != "tier2s_gpu23_execution_erratum1"
        or erratum.get("research_mode") != "exploratory_source_only"
        or erratum.get("parent_protocol", {}).get("sha256")
        != PARENT_PROTOCOL_SHA256
        or erratum.get("parent_governance_amendment", {}).get("sha256")
        != PARENT_REGISTRATION_SHA256
        or erratum.get("parent_launcher", {}).get("sha256")
        != PARENT_LAUNCHER_SHA256
        or idle.get("driver_bookkeeping_ceiling_mib") != 4
        or idle.get("corrected_definition")
        != "memory_used_mib_at_most_4_on_each_selected_gpu_and_no_compute_processes"
        or test_phase.get("old_test_or_registration_mutated") is not False
        ...
        or append_only.get("parent_protocol_mutated") is not False
        or append_only.get("parent_governance_amendment_mutated") is not False
        or append_only.get("parent_launcher_mutated") is not False
        or append_only.get("replacement_scope")
        != "host_idle_predicate_and_post_registration_test_phase_only"
    ):
        raise RuntimeError("Tier2S GPU2/3 execution erratum semantics drift")
    return {
        "verified": True,
        "parent_governance_binding": parent,
        "append_only_state": _verify_parent_plan_is_unexecuted(),
...
def _registration_payload(
    ...
    return {
        ...
        "registered_at": _now(),
        "decision": "AUTHORIZE_TIER2S_V2_GPU23_EXECUTION_ERRATUM1_SOURCE_ONLY",
        ...
        "execution_scope": {
            ...
            "utilization_is_recorded_not_a_standalone_busy_signal": True,
        },
        "authorization": {
        ...
        "failure_policy": {
            ...
            "gpu_memory_above_4mib_or_any_compute_process": "FAIL_CLOSED_NO_PREFLIGHT_OR_LAUNCH",
            ...
            "superseded_v2_launch_intent_appears": "FAIL_CLOSED_NEW_ERRATUM_REQUIRED",
        },
    }
...
def verify_registration() -> dict[str, Any]:
    ...
    if (
        ...
        != "AUTHORIZE_TIER2S_V2_GPU23_EXECUTION_ERRATUM1_SOURCE_ONLY"
        ...
        or execution.get("utilization_is_recorded_not_a_standalone_busy_signal")
        is not True
...
def require_frozen_execution_erratum(
    *, expected_registration_sha256: str | None = None
    ...
    return {
        ...
        "parent_tier2s_governance_registration": parent["registration"],
        "parent_aaai27_governance_registration": parent["parent_governance_registration"],
