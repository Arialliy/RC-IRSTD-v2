...
uses two fixed FIFO GPU lanes on physical GPUs 2/3, and delegates all numerical conclusions to a
...
from scripts import register_tier2s_gpu23_amendment_v2 as governance  # noqa: E402
...
SCHEMA = "rc-irstd-aaai27-tier2s-factorized-coordinator-v2-gpu23"
PROTOCOL_SCHEMA = (
...
TARGET_DENIAL_SCHEMA = "rc-irstd-aaai27-tier2s-outer-target-denial-v2-gpu23"
EVENT_SCHEMA = "rc-irstd-aaai27-tier2s-scheduler-event-v2-gpu23"
STATUS_SCHEMA = "rc-irstd-aaai27-tier2s-factorized-status-v2-gpu23"
...
def _expected_lane(seed: int, role: str, fold: str, scope: str) -> int:
    # Frozen balanced 9/9 assignment over the exact physical GPU 2/3 allowlist.
    within_seed = {
    ...
    lane_ordinal = (base + (seed - 43)) % 2
    return PHYSICAL_GPUS[lane_ordinal]
        ...
        for role, fold, scope in specs:
            ...
            jobs.append(
                ExportJob(
                    ...
                    physical_gpu=gpu,
                    container_gpu_ordinal=CONTAINER_ORDINAL_BY_PHYSICAL[gpu],
                    queue_index=queue_index,
                    output_dir=str(output),
                )
            )
    if len(jobs) != 18 or lane_counts != {2: 9, 3: 9}:
...
def verify_prerequisites() -> dict[str, Any]:
    ...
    if (
        ...
        or protocol.get("execution", {}).get("container_logical_ordinals")
        != {"2": 0, "3": 1}
        or protocol.get("execution", {}).get("allow_gpu_fallback") is not False
    ...
    required_code = (
        ...
        Path(governance.__file__).resolve(),
        EXPORTER,
        EVALUATOR,
    ...
    return {
        ...
        "physical_to_container_ordinal": {
            str(gpu): CONTAINER_ORDINAL_BY_PHYSICAL[gpu] for gpu in PHYSICAL_GPUS
        },
...
def register(packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    target_sha = _write_once_json(
        TARGET_LOCK,
        {
            "schema_version": TARGET_DENIAL_SCHEMA,
            "protocol_id": PROTOCOL_ID,
            "governance_binding": packet["governance_binding"],
            "tier2s_preregistration_binding": tier2s_preregistration_binding,
            "outer_target_access_authorized": False,
...
def _child_env(job: ExportJob) -> dict[str, str]:
    env = os.environ.copy()
    expected_ordinal = CONTAINER_ORDINAL_BY_PHYSICAL.get(job.physical_gpu)
    if expected_ordinal is None or job.container_gpu_ordinal != expected_ordinal:
        raise RuntimeError("Tier2S physical/container GPU mapping drift")
    env["CUDA_VISIBLE_DEVICES"] = str(job.container_gpu_ordinal)
...
def _run_lane(
    ...
    for job in sorted(jobs, key=lambda value: value.queue_index):
        if (
            ...
            or job.container_gpu_ordinal != CONTAINER_ORDINAL_BY_PHYSICAL.get(gpu)
        ):
            raise RuntimeError("Tier2S lane GPU identity drift")
        ...
        events.append(
            {
                ...
                "run_id": job.run_id,
                "physical_gpu": gpu,
                "container_gpu_ordinal": job.container_gpu_ordinal,
